...
`DAD-drift` is written in `Fortran 90`, the [source code](source_code/) contains all `source files` of the model, [make](source_code/make) containes two `Makefiles` for compilation of the executable using `ifx` or `gfortran`.
...
The **DAD-drift** model has been described and applied in the following peer-reviewed publications:
